...
public class IaSHandlerDistillationHeat implements IIaSApiDistillable {
	...
	public ItemStack getDistillationYield(ItemStack target) {
		int time = getTime(target);
		if(time >= 12800)
			return new ItemStack(NyxItems.heat, Math.min(64, time/12800*target.stackSize),3);
		if(time % 3200 == 0)
			return new ItemStack(NyxItems.heat, Math.min(64, time/3200*target.stackSize),2);
		if(time > 3200)
			return new ItemStack(NyxItems.heat, Math.min(64, time/800*target.stackSize),1);
		if(time % 800 == 0)
			return new ItemStack(NyxItems.heat, Math.min(64, time/800*target.stackSize),1);
		return new ItemStack(NyxItems.heat, Math.min(64, time/200*target.stackSize),0);
	}
