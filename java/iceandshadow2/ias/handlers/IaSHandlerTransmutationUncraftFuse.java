...
/**
 * Reverses crafting recipes that obey three rules: 1: The recipe must generate
 * at least double the output items from input items. 2: The recipe must be a
 * standard/oredict shaped/shapeless recipe. 3: None of the inputs from the
 * recipe can be on the uncraft blacklist.
 */
public class IaSHandlerTransmutationUncraftFuse implements IIaSApiTransmute {
	...
	protected IntPair costs(IRecipe recipe, ItemStack target, ItemStack catalyst, boolean sanitycheck) {
		if (recipe.getRecipeOutput() == null)
			return new IntPair();
		final int sizeOut = recipe.getRecipeOutput().stackSize;
		if (sanitycheck) {
			if (recipe.getRecipeSize() >= sizeOut)
				return new IntPair();
			if (!target.isItemEqual(recipe.getRecipeOutput()) || target.hasTagCompound() || catalyst.hasTagCompound())
				return new IntPair();
			if (recipe.getClass() != ShapelessRecipes.class && recipe.getClass() != ShapelessOreRecipe.class
					&& recipe.getClass() != ShapedRecipes.class && recipe.getClass() != ShapedOreRecipe.class)
				return new IntPair();
		}
		for (final Object o : IaSCraftingHelper.getCraftingRecipeInputs(recipe))
	...
	public List<ItemStack> getTransmuteYield(ItemStack target, ItemStack catalyst, World world) {
		...
		do {
			...
			for (final Object o : ins)
				try {
					final ItemStack is = IaSCraftingHelper.extractStack(o);
