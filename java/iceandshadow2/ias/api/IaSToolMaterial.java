...
public abstract class IaSToolMaterial implements IIaSApiSacrificeXp, IIaSAspect {
	...
	/**
	 * Called by the default implementation of onAttack to determine how much damage
	 * a tool takes from attacking.
	 *
	 * @param is
	 * @param user
	 * @param target
	 ...
	 * forced to override it if it won't be used. Cheat sheet: 0 = wood, 1 = stone,
	 * 2 = iron, 3 = diamond/echir
	 *
	 * @return
	 */
	 ...
	 * getHarvestLevel(). Cheat sheet: -1 = fists, 0 = wood, 1 = stone, 2 = iron, 3
	 * = diamond
	 */
	...
	/**
	 * Called when a tool breaks to determine whether or not a "broken" tool item
	 * should be given.
	 *
	...
	/**
	 * Called to determine harvest effectiveness. In the future, will be used to add
	 * fortune-like effects to harvesting.
	 *
	 * @param bl
	 * @param is
	 * @return -1 if this tool cannot harvest a given block, 0 if the tool should
	 *         behave as normal, and anything greater than 0 if the tool can harvest
	 *         the specified block.
	 */
	 ...
	 * Cheat sheet: -1 = fists, 0 = wood, 1 = stone, 2 = iron, 3 = diamond Default
	 * implementation mimics vanilla behavior.
	 *
	 * @param is        The stack.
	...
	/**
	 * Gets a thrown throwing knife's damage bonus against certain entities. This is
	 * NOT called by left-click attacks.
	 *
	 * @param knife  The in flight knife.
	 * @param user   The user.
	 * @param target The target.
	 * @return The damage that the knife should do. Note that this is the full
	 *         physical damage, not bonus damage, so returning 0.0F will cause the
	 *         tool to do no damage.
	 */
	...
	/**
	 * Called to get the damage source for a throwing knife. Useful for doing things
	 * like making a throwing knife's damage pierce armor.
	 *
	...
	/**
	 * Called when a throwing knife collides with an entity or block and onKnifeHit
	 * returns true. Used to determine the item stack that the knife should drop.
	 *
	 * @param user  The thrower.
	 * @param knife The throwing knife entity.
	 * @return The item stack to drop when the knife hits something, or null if no
	 *         drop.
	 */
	...
	/**
	 * Gets an item associated with this material (ex Echir Ingot with Echir, Cortra
	 * with Cortra).
	 */
	 ...
	 * something like "IceAndShadow2:iasTool". The tool class and tool material name
	 * would then be tacked on to that name. The final string texture ends up
	 * looking something like "IceAndShadow2:iasToolEchirPickaxe".
	 */
	...
	/**
	 * Gets the tool's damage bonus against certain entities. This is NOT called by
	 * in-flight throwing knives, but is called on throwing knife left-click.
	 * Default implementation emulates vanilla damage variances between tools.
	 *
	 * @param is     The tool.
	 * @param user   The user.
	 * @param target The target.
	 * @return The damage that the tool should do. Note that this is pure damage,
	 *         not bonus damage, so returning 0.0F will cause the tool to do no
	 *         damage.
	 */
	...
	/**
	 * Gets the item required in the transmutation altar to transmute a tool/weapon
	 * to this. Used for cases where the transmutation itself is quite basic.
	 *
	...
	/**
	 * Called whenever the tool does is swung at an entity. This is NOT called when
	 * a thrown knife hits its target.
	 *
	 ...
	 * @return The number of points of durability that should be deducted by this
	 *         left-click. This is ignored by throwing knives.
	 */
	...
	/**
	 * Called when a player uses the right click of a tool. This does NOT get called
	 * when using throwing knives. Use onThrowingKnifeThrow() instead.
	 *
	 * @param is   The item stack being used.
	 * @param user The user of the item stack.
	 * @return The number of points of durability that should be deducted by this
