...
public class IaSArmorMaterial {
	...
	public IaSArmorMaterial(String name, Item material, int durabilitymod, int head, int torso, int legs, int feet, int enchantability) {
	 ...
	 * @return
	 */
	public ArmorMaterial getArmorStats() {return m;}
	
	/**
	 * Gets the armor coverage as a function of what slot the armor is in and its durability.
	 * @param slot The slot the armor is in. Cheat sheet: 1 - Head, 4 - Feet.
	 * @param durability The armor's durability, expressed as a value from 0 to 1, where 1 is fully repaired.
	 * @return The coverage provided by that piece of armor. The cumulative coverage from a full set of same-type armor at full durability should be 10.
	 */
	public double getCoverage(int slot, double durability) {
		switch(slot) {
		case 1: return 2*Math.cbrt(durability);
		case 2: return 4*Math.cbrt(durability);
		case 3: return 3*Math.cbrt(durability);
		case 4: return 1;
		default: return 0;
		}
	}
	 ...
	 * @param coverage A value from 1-10 representing how much of that type of armor the player has equipped.
	...
	/**
	 * Called by Nyxian armors upon their wearer taking damage. Called one for each type of armor the player wears.
	 * @param coverage A value from 1-10 representing how much of that type of armor the player has equipped.
	 * @param major Whether the player has major coverage. Given for convenience.
