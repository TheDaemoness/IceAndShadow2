...
public class IaSBlockHelper {
	...
	/**
	 * Forcibly harvests the specified block. Does NOT spawn the block's items,
	 * rather leaving that task up to the callee. Note that blocks that perform
	 * additional actions in harvestBlock may malfunction.
	 * 
	 * @param fortune The fortune level. Negative levels imply silk touch with a
	 *                fortune level = |fortune|-1
	 */
	public static List<ItemStack> harvest(EntityPlayer breaker, int x, int y, int z, int fortune, float chance,
			boolean hunger) {
		...
		if (!w.isRemote && !w.restoringBlockSnapshots) {
			...
			chance = ForgeEventFactory.fireBlockHarvesting(items, w, bl, x, y, z, meta, fortune, chance, silktouch,
					breaker);
