...
public class Sculptor {
	...
	/**
	 * Create the vertical corners of a cube centered at x, y, and z (no corners).
	 *
	 * @param world
	 * @param xSub
	 * @param y
	 * @param zSub
	 * @param xdelta    The number of blocks to remove before or after the specified
	 *                  x coordinate.
	 * @param ydelta    The number of blocks to remove before or after the specified
	 *                  y coordinate.
	 * @param zdeltaThe number of blocks to remove before or after the specified z
	 *                  coordinate.
	 */
	...
	public static void cylinder(World world, int x, int y, int z, double radius, int height, Block bl, int meta) {
		...
		final ISupplierBlock bli = new SupplierBlockPosCuboid(-intradius, 0, -intradius, intradius, height, intradius);
	...
	public static void dome(World world, int x, int y, int z, double radius, Block bl, int meta) {
		final int intradius = (int) Math.ceil(radius);
		final ISupplierBlock bli = new SupplierBlockPosCuboid(-intradius, 0, -intradius, intradius, intradius,
				intradius);
	...
	public static void sphere(World world, int x, int y, int z, int radius, Block bl, int meta) {
		final int intradius = (int) Math.ceil(radius);
		final ISupplierBlock bli = new SupplierBlockPosCuboid(-intradius, -intradius, -intradius, intradius, intradius,
				intradius);
		bli.addFilter(new BlockFilterRadius<BlockPos3>(new BlockPos3(x, y, z), radius));
		bli.offsetCenter(x, y, z);
		for (final BlockPos3 bp3 : bli)
	 ...
	 * @param y
	 * @param zSub
	 * @param xdelta The number of blocks to extrude before or after the specified x
	 *               coordinate.
	 * @param ydown  The number of blocks to fill upward from.
	 * @param zdelta The number of blocks to extrude before or after the specified z
	 *               coordinate.
	 */
	...
	public static void walls(World world, int xLow, int yLow, int zLow, int xHigh, int yHigh, int zHigh, Block bl,
			int meta) {
		final IteratorConcatBlock bic = new IteratorConcatBlock();
		bic.add(new SupplierBlockPosCuboid(xLow + 1, yLow, zLow, xHigh - 1, yHigh, zLow));
		bic.add(new SupplierBlockPosCuboid(xLow + 1, yLow, zHigh, xHigh - 1, yHigh, zHigh));
		bic.add(new SupplierBlockPosCuboid(xLow, yLow, zLow + 1, xLow, yHigh, zHigh - 1));
		bic.add(new SupplierBlockPosCuboid(xHigh, yLow, zLow + 1, xHigh, yHigh, zHigh - 1));
		for (final BlockPos3 bp : bic)
			bp.block(world, bl, meta);
	}
}
