 ...
 * implemented by an item that will go in the target slot of a transmutation
 * altar. They may also be implemented by functions These functions are only
 * called if the item in the target slot implements this. In other words, no
 * making lenses out of objects that the mod creator didn't specifically intend.
...
public interface IIaSApiTransmuteLens {
	 ...
	 * @param lens
	 *            The equipped lens.
	 * @return The icon to be used for the altar while this lens is equipped, or
	 *         null if no change.
	 */
	public IIcon getAltarTopTexture(ItemStack lens);

	/**
	 * Gets the transmutation duration. Also used to check if a transmutation is
	 * possible.
	 *
	 * @param lenstype
	 *            The type of the equipped lens. Null indicates a third-party
	 *            lens.
	 * @param lens
	 *            The equipped lens. Used to check if a transmutation should be
	 *            possible with this lens.
	 * @param target
	 *            The item stack being distilled.
	 * @return The number of ticks it takes to process the item stack, or 0 if
	 *         not possible.
	 */
	public int getTransmuteLensRate(EnumIaSLenses lenstype, ItemStack lens, ItemStack target);

	/**
	 * Gets the yield of the transmutation. As a rule of thumb, the lens should
	 * NOT be consumed by the transmutation.
	 *
	 * @param lens
	 *            The equipped lens. Provided for mod authors who want to do fun
	 *            things with NBT.
	 * @param target
	 *            The item stack being transmuted.
	 * @return The item stacks yielded from doing the transmutation. Null is
	 *         acceptable.
	 */
