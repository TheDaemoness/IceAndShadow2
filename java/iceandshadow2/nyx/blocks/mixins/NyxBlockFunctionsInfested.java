...
public class NyxBlockFunctionsInfested {
	public static AxisAlignedBB getCollisionBoundingBoxFromPool(
			...
			for (int yit = -1; yit <= 1; ++yit) {
				for (int zit = -1; zit <= 1; ++zit) {
					...
					if (b.isLeaves(w, x + xit, y + yit, z + zit)) {
						w.setBlock(x + xit, y + yit, z + zit,
								NyxBlocks.infestLeaves);
					} else if (b.isWood(w, x + xit, y + yit, z + zit)) {
						final int meta = w.getBlockMetadata(x + xit, y + yit, z
								+ zit) & 12;
						w.setBlock(x + xit, y + yit, z + zit,
								NyxBlocks.infestLog, meta, 0x2);
