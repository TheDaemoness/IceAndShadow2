...
public class NyxBlockDistiller extends IaSBaseBlockSingle {
	...
	@Override
	public int onBlockPlaced(World w, int x,
			...
			if(b instanceof BlockChest) {
				...
				for(; mov < 9; mov += 2) {
					if(w.getTileEntity(x+mov/3-1, y+1, z+mov%3-1) instanceof TileEntityChest) {
						tent2 = (TileEntityChest)w.getTileEntity(x+mov/3-1, y+1, z+mov%3-1);
						break;
					}
				}
				...
				if(tent2 != null)
					w.markTileEntityChunkModified(x+mov/3-1, y+1, z+mov%3-1, tent2);
			}
				...
				for(int i = 0; i < hop.getSizeInventory(); ++i) {
					ItemStack lst = processStack(hop.getStackInSlot(i));
					if(lst != null) {
						hop.decrStackSize(i, getStackDecr(hop.getStackInSlot(i)));
						depositStack(lst, w, x, y-1, z);
						break;
					}
				}
	...
	protected void depositStack(ItemStack is, World w, int x, int y, int z) {
		...
		if(b.hasTileEntity(w.getBlockMetadata(x, y, z))) {
			if(b instanceof BlockChest) {
				TileEntityChest tent = (TileEntityChest)w.getTileEntity(x, y, z);
				TileEntityChest tent2 = null;
				int mov = 1;
				for(; mov < 9; mov += 2) {
					if(w.getTileEntity(x+mov/3-1, y, z+mov%3-1) instanceof TileEntityChest) {
						tent2 = (TileEntityChest)w.getTileEntity(x+mov/3-1, y, z+mov%3-1);
						break;
					}
				}
				int stacksize = tent.getSizeInventory();
				if(tent2 != null)
					stacksize = Math.max(stacksize, tent2.getSizeInventory());
				for(int i = 0; i < stacksize; ++i) {
					if(i < tent.getSizeInventory()  && tent.isItemValidForSlot(i, is)) {
						if(tent.getStackInSlot(i) != null) {
							ItemStack iis = tent.getStackInSlot(i);
							int size = Math.min(iis.getMaxStackSize()-iis.stackSize,is.stackSize);
							if(size <= 0)
								continue;
							ItemStack nis = is.splitStack(size);
				...
				if(tent2 != null)
					w.markTileEntityChunkModified(mov/3-1, y, mov%3-1, tent2);
				return;
			}
		}
