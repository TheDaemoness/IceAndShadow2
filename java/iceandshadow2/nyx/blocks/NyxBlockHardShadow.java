		...
		for(int xit = -1; xit <= 1; ++xit) {
			for(int zit = -1; zit <= 1; ++zit) {
				for(int yit = -1; yit <= 1; ++yit) {
					...
					power = Math.max(power, bl.isProvidingStrongPower(w, x+xit, y+yit, z+zit, w.getBlockMetadata(x+xit, y+yit, z+zit)));
					power = Math.max(power, bl.isProvidingWeakPower(w, x+xit, y+yit, z+zit, w.getBlockMetadata(x+xit, y+yit, z+zit)));
				}
			}
		}
