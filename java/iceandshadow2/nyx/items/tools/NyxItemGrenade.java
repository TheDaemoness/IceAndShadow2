...
public class NyxItemGrenade extends IaSBaseItemMultiGlow {
	...
	public String getItemStackDisplayName(ItemStack is) {
		...
		final String original = LanguageRegistry.instance().getStringLocalization("item." + getModName() + suffix);
		return original + " ("
				+ LanguageRegistry.instance().getStringLocalization("ias2.grenade." + explosiveReasoning.getName())
				+ ")";
	}
	...
	public String getLocalizedHintArgument(EntityPlayer entityPlayer, ItemStack itemStack) {
		...
		final String hint = LanguageRegistry.instance()
				.getStringLocalization(Math.abs(fusetime) == 20 ? "ias2.unit.second" : "ias2.unit.seconds");
		return fuse + " " + hint;
	}
	...
	public void onPlayerStoppedUsing(ItemStack is, World w, EntityPlayer pl, int time) {
		...
		if (!w.isRemote) {
			final EntityGrenade eg = new EntityGrenade(w, pl, getGrenadeLogic(is).getId(), time);
