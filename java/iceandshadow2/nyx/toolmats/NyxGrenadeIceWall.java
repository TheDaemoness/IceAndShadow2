...
public class NyxGrenadeIceWall extends IaSGrenadeLogic {
	...
	public void onDetonate(EntityGrenade ent) {
		final int xzLim = 2, x = (int) Math.floor(ent.posX), y = (int) ent.posY, z = (int) Math.floor(ent.posZ);
	...
	public void onSpawnParticle(World w, double x, double y, double z) {
		IaSFxManager.spawnParticle(w, "cloudLarge", x, y, z, 0.5 - w.rand.nextDouble(), w.rand.nextDouble() / 4,
				0.5 - w.rand.nextDouble(), true, false);
		IaSFxManager.spawnParticle(w, "cloudLarge", x, y, z, 0.5 - w.rand.nextDouble(), w.rand.nextDouble() / 4,
				0.5 - w.rand.nextDouble(), true, true);
	}
