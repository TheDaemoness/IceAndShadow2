	...
	public void addTooltip(List l, String localized, boolean isWarning) {
		if (localized != null && !localized.isEmpty()) {
			final String format = isWarning
					? (EnumChatFormatting.DARK_RED.toString() + EnumChatFormatting.UNDERLINE.toString())
					: (EnumChatFormatting.GRAY.toString());
			l.add(format + EnumChatFormatting.ITALIC.toString() + localized + EnumChatFormatting.RESET);
		}
	}

	@SubscribeEvent
	public void onTooltip(ItemTooltipEvent e) {
		final Object it = IaSCraftingHelper.extractItem(e.itemStack);
		boolean isWarning = false;
		if (it instanceof IIaSDescriptive) {
			final IIaSDescriptive desc = ((IIaSDescriptive) it);
			String localized = null;
			String unlocalized = ((IIaSDescriptive) it).getUnlocalizedHint(e.entityPlayer, e.itemStack);
			if (unlocalized != null && !unlocalized.isEmpty()) {
				isWarning = ((IIaSDescriptive) it).isHintWarning(e.entityPlayer, e.itemStack);
			...
			if (GameSettings.isKeyDown(Minecraft.getMinecraft().gameSettings.keyBindSneak)) {
				...
				unlocalized = ((IIaSDescriptive) it).getUnlocalizedDescription(e.entityPlayer, e.itemStack);
				if (unlocalized != null && !unlocalized.isEmpty())
