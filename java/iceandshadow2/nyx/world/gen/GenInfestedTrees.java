...
public class GenInfestedTrees extends WorldGenerator {
    ...
    {
        int[] aint = new int[] {this.basePos[0], this.basePos[1]+2, this.basePos[2]};
        int[] aint1 = new int[] {this.basePos[0], this.basePos[1] + this.heightLimit - 1, this.basePos[2]};

        Block soil = this.worldObj.getBlock(this.basePos[0], this.basePos[1] - 1, this.basePos[2]);
        boolean isValidSoil = (soil != null && soil == Blocks.snow);
        if (!isValidSoil)
        {
            return false;
        }
	...
	private void generateBerryPod() {
		int h = 2+this.rand.nextInt(3);
		int dir = this.rand.nextInt(4);
		int x = this.basePos[0], y = this.basePos[1]+h, z = this.basePos[2];
		int meta = dir;
		if(dir == 0)
			--z; //North
		else if(dir == 1)
