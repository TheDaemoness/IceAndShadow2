...
public class GenRuinsMines extends GenRuins {
	...
	protected boolean genArm(World w, Random r, int x, int y, int z, ForgeDirection dir, int antichance) {
		...
		default:
			...
			if (antichance > 6 || r.nextInt(antichance + 3) > 3)
				return false;
			final int[] slope = { 0, r.nextInt(2), r.nextInt(3) / 2 },
					slant = { (r.nextInt(5) - 2) / 2, r.nextInt(5) - 2, r.nextInt(3) - 1 };
			for (int i = 0; i < length; ++i) {
				final int slopeIndex = (i / (1 + length / slope.length)),
					...
					for (int zi = -6; zi <= 6; ++zi)
						for (int yi = -2; yi <= 5; ++yi) {
							...
							if (bl == Blocks.bedrock) {
								++y;
								slope[slopeIndex] = 0;
							} else if (bl == NyxBlocks.infestSpawner
									&& w.getBlockMetadata(x + xi, y + yi, z + zi) == 0) {
								w.setBlock(x + xi, y - 3, z + zi, NyxBlocks.infestSpawner);
								w.setBlockToAir(x + xi, y + yi, z + zi);
							}
						}
				Sculptor.dome(w, x - 1 + r.nextInt(3), y, z - 1 + r.nextInt(3), r.nextInt(3) == 0 ? 4 : 3, Blocks.air,
						0);
				if (r.nextInt(14) == 0 && !nospawner) {
					...
					nospawner = false;
				final ForgeDirection slantMod = dir
						.getRotation(slant[slantIndex] < 0 ? ForgeDirection.DOWN : ForgeDirection.UP);
	...
	protected void genRewardsChest(World w, Random r, int x, int y, int z, EnumRarity quality) {
		...
		if (IaSBlockHelper.isAir(bl) || bl instanceof NyxBlockRail)
			--y;
		Sculptor.cube(w, x - 1, y - 1, z - 1, x + 1, y, z + 1,
				quality == EnumRarity.common ? NyxBlocks.brickExousic : Blocks.obsidian, 0);
			...
			if (rewardid < 5 && rareflag) {
				...
				if (quality == EnumRarity.common || quality == EnumRarity.uncommon) {
					item = IaSTools.setToolMaterial(IaSTools.pickaxe,
							quality == EnumRarity.uncommon && r.nextBoolean() ? (r.nextBoolean() ? "Devora" : "Cortra")
									: "Echir");
	...
	protected void makeRails(World w, ForgeDirection toward, int xP, int yP, int zP, int xN, int yN, int zN) {
		...
		while (xRequirement != 0 || zRequirement != 0) {
			final Block space = w.getBlock(x + toward.offsetX, y, z + toward.offsetZ),
		...
		if (curve == null || curve == toward || curve == toward.getOpposite()) {
			if (IaSBlockHelper.isAir(support)) {
				if (!IaSBlockHelper.isAir(w.getBlock(x, y - 2, z))) {
					switch (toward) {
					case NORTH:
						w.setBlock(x, y - 1, z, NyxBlocks.rail, 5, 2);
						break;
					case SOUTH:
						w.setBlock(x, y - 1, z, NyxBlocks.rail, 4, 2);
						break;
					case WEST:
						w.setBlock(x, y - 1, z, NyxBlocks.rail, 2, 2);
						break;
					case EAST:
						w.setBlock(x, y - 1, z, NyxBlocks.rail, 3, 2);
						break;
					default:
						break;
					}
			...
			int md = 5;
			for (int i = 0; i < 4; ++i) {
				final ForgeDirection ew = ((i + 1) & 2) == 1 ? ForgeDirection.WEST : ForgeDirection.EAST,
