...
public class NyxBiomeProvider {
	...
	public static void fillArray(long seed, BiomeGenBase[] biomeArray, int xchunk, int zchunk) {
		...
		final long x = ((long) xchunk << 4), z = ((long) zchunk << 4);
		final int xBoxBase = (int) ((x + SCALE / 2) / SCALE), zBoxBase = (int) ((z + SCALE / 2) / SCALE);
		for (int i = 0; i < 16; ++i)
			points[i] = getBiomePoint(seed, xBoxBase - 1 + (i & 3), zBoxBase - 1 + (i >> 2));
		for (int i = 0; i < 256; ++i) {
			final long xFull = x + (i & 15), zFull = z + (i >> 4);
			final int xBox = (int) ((xFull + SCALE / 2) / SCALE), zBox = (int) ((zFull + SCALE / 2) / SCALE);
			biomeArray[i] = getBiomeAtPoint(seed, xFull, zFull, points, xBox - xBoxBase, zBox - zBoxBase);
		}
	}

	protected static BiomeGenBase getBiomeAtPoint(long seed, long x, long z, BiomePoint[] points, int xDelta,
			int zDelta) {
	...
	protected static NyxBiome getBiomeForBoxPoint(Random r, int biomeNum, long x, long z) {
		...
		final int distance = (int) (IaSWorldHelper.distance2(x, z) / 2), type = r.nextInt(3);
		if (biomeNum == 0) {
			final boolean altMountains = r.nextBoolean(), severeMountains = distance > 2000 && r.nextBoolean();
	...
	protected static int getNumForBox(long seed, Random r, int xBox, int zBox) {
		...
		final int aD = (int) ((seed * 1607) & 6) >> 1, bD = (int) ((seed * 1352) & 6) >> 1;
		final int a = xBox + zBox * 2, b = zBox - xBox * 2;
		final boolean am = (a + aD) % 3 == 0, bm = (b + bD) % 3 == 0;
		return (am && bm ? 1 : 0);
	}
}
